	...
	for (let idx = 0; idx < selected.dims[1]; idx++) {
		...
		const maskConfig = new ort.Tensor(
			...
			new Float32Array([
				...
				w, // upscale width
				h, // upscale height
				...Colors.hexToRgba("#000000", 255), // color in RGBA
			])
		); // mask config
		...
		objects.push({
			...
			probability: score,
			color: color,
			mask: await createImageBitmap(new ImageData(new Uint8ClampedArray(mask_mat.data), mask_mat.cols, mask_mat.rows)),
			ratio: [xRatio, yRatio],
			scale: [image.width / modelWidth, image.height / modelHeight],
			bounding: [x, y, w, h], // upscale box
		});
...
imageInput.onchange = function(evt) {
	...
	if (FileReader && files && files.length) {
		...
		fr.onload = function () {
			...
			img.src = fr.result;
			inputImg.src = fr.result;
			inputCanvas.setWidth(img.naturalWidth);
			inputCanvas.setHeight(img.naturalHeight);


			setTimeout(async () => {
				let objects = await detectImage(
					inputImg,
					mySession,
					topk,
					iouThreshold,
			...
			//inputCanvas.centerObject(imgInstance);
...
let inputCanvas = new fabric.Canvas('input', { isDrawingMode: true });
...
function createStickerFromMask() {
	...
	console.log(`x: ${mx}, y: ${my}, w: ${mx2-mx}, h: ${my2-my}`);
	...
	let maskCtx = inputCanvas.toCanvasElement().getContext("2d");
	let croppedMask = maskCtx.getImageData(x, y, w, h);
